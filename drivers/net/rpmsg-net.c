...
#define RPMSG_NET_L2_MTU            (MAX_RPMSG_PAYLOAD_SIZE > 1492 ? 1492 : MAX_RPMSG_PAYLOAD_SIZE)
...
static void net_setup(struct net_device *ndev)
{
    ...
    ndev->tx_queue_len      = 5; //DEFAULT_TX_QUEUE_LEN;
...
static void net_xmit_work_handler(struct work_struct *work)
{
    ...
    if (err) {
		...
		priv->ndev->stats.tx_aborted_errors++;
        if (priv->is_delayed) {
    ...
    priv->ndev->stats.tx_bytes += priv->skb->len;
